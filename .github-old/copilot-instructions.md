...
## 專案特定慣例
- 游標規則：自訂自動化和指引存在於`.cursor/rules/*.mdc`中。 範例： `.cursor/rules/docs-lint.mdc` （lint程式）、`.cursor/rules/generate-release-notes.mdc` （編輯`help/user-guide/release-notes.md`時指向發行說明技能）。 請遵循這些步驟以自動執行工作。
- 代理程式技能：任務工作流程在`.cursor/skills/<name>/SKILL.md`下進行。 範例： `.cursor/skills/generate-release-notes/SKILL.md` （完整的GenStudio發行說明程式；在相同的資料夾中搭配`examples.md`和`reference.md`使用）。
- 檔案名稱和前置內容：
   - 發行說明：保留`help/user-guide/release-notes.md`上現有的Experience League frontmatter （請參閱`.cursor/skills/generate-release-notes/reference.md#frontmatter`）；精簡規則`.cursor/rules/generate-release-notes.mdc`會在編輯該檔案時自動附加。
...
## 編輯的安全性規則（AI應該做什麼）
- 切勿將Jira ID、內部連結或僅限公司的參照新增至公開檔案。 請參閱`.cursor/skills/generate-release-notes/SKILL.md` （區段&#x200B;**禁止的內容**）。
