...
[!DNL Activate]現在支援大量啟用：從[!DNL Content]中選取多個已核准的體驗，並在單一啟用表格中跨多個付費廣告頻道發佈這些體驗，包括Meta、LinkedIn、Google Campaign Manager 360、Amazon Ads、Innovid、TikTok、YouTube、ChatGPT和The Trade Desk。 編輯內嵌或大量共用欄位，然後發佈每個廣告以及每個廣告狀態追蹤，並對失敗的任何廣告自動重試。

深入瞭解[啟動工作流程](/help/user-guide/activation/create-activation.md)。
...
短格式視訊現在是[!DNL GenStudio for Performance Marketing]中受控制的頻道。 在[!DNL Create]工作流程中建立並啟用品牌上YouTube Shorts，從一開始即套用品牌、產品和角色護欄。

啟用無需手動切換即可關閉回圈：將Shorts推送到Google Ads Demand Gen行銷活動中，或當您的團隊透過DV360流量YouTube廣告時，將其上傳為YouTube上的未列出影片。

### CM360視訊啟用

在[!DNL GenStudio for Performance Marketing]中建立的視訊顯示廣告不再於CM360啟動閘道停止。 從顯示頻道選取控管視訊體驗，在啟動工作流程中預覽，並隨影像和HTML5格式將[創意內容直接發佈至Google Campaign Manager 360](/help/user-guide/activation/activate-cm360-ad.md)。

### 在[!DNL Activate]中的Meta應用程式促銷活動

行動應用程式行銷人員現在可以[透過「應用程式促銷」目標，為Meta行銷活動](/help/user-guide/activation/activate-meta-ad.md)啟用體驗，包括鎖定應用程式安裝的廣告集和應用程式事件。 之前，僅支援非應用程式行銷活動目標。

### Horizon畫布上的翻譯

現成翻譯功能現在延伸至Horizon Canvas上的付費媒體體驗，因此區域團隊可將已核准的Meta、LinkedIn和顯示草稿當地語系化，而無需將復本匯出至外部工具。

如需更廣泛的翻譯工作流程，請參閱[翻譯及本地化體驗](/help/user-guide/create/translate-experiences.md)。

### 電子郵件範本中每個Pod有多個影像

[!DNL Create]中的電子郵件範本現在可以在單一Pod中攜帶多個影像，為設計人員提供更豐富的英雄版面配置、產品相簿和多資產storytelling的空間，而不需將內容分割到額外的區段。
...
Horizon畫布上的付費媒體草稿不再是單獨練習。 擁有者現在可以透過電子郵件邀請具有&#x200B;**[!UICONTROL 編輯者]**&#x200B;或&#x200B;**[!UICONTROL 檢視者]**&#x200B;角色的共同作業人員，讓每個人都從相同的即時草稿中工作，而不是在工具之間退回匯出。

編輯們使用畫布中的[!UICONTROL 共用]按鈕，即時觀看工作畫板和欄位更新（包括標題、CTA和說明）登陸。

### 已重新整理[!DNL Create] [!DNL Content]頁面上的卡片配置

[!DNL Content]內的[!DNL Create]進入點會以重新整理的卡片版面配置進行重新開機。

### AEM內容片段在HTML畫布上的電子郵件體驗中交換

HTML畫布上的[電子郵件體驗](/help/user-guide/create/email-experiences.md)現在提供內容交換功能，針對預先控管的模組化內容，與預先核准的&#x200B;**AEM內容片段**&#x200B;交換；不會從側邊管道貼入。

信賴貴組織已發佈在AEM中的發酵，而不需離開GenStudio for Performance Marketing。

### 從URL新增[!DNL Brands]、[!DNL Products]和[!DNL Personas]

[!DNL GenStudio for Performance Marketing]內嵌直接從URL發佈訊號&#x200B;**[!DNL Brand]**、**[!DNL Product]**&#x200B;和&#x200B;**[!DNL Persona]**，壓縮手動輸入的小時數，同時讓輸出內容以品牌實際線上上發佈的內容為基礎。 [新增公開URL的指引](/help/user-guide/guidelines/add-guidelines.md#guidance-when-adding-from-a-url)最後會反映行銷人員在開啟簡介之前研究網頁的方式。

品牌指導方針會以單一通道進行，而產品和角色會從智慧型短清單開始，因此您可以在系統豐富記錄之前組織所屬內容。 AI輔助探索會在網站較薄或閘道時刻意保持保守，這可以在保護受規管團隊的同時不阻礙動能。

需要&#x200B;**[!UICONTROL Brand Manager]**&#x200B;或更高版本的組織仍會自動強制執行這些護欄。 當您跨地區或產品組合擴展計畫時，可公開存取的品牌網域會繼續解鎖最豐富的指引。

### ChatGPT廣告現在可在[!DNL Create]、[!DNL Content]和[!DNL Activate]中使用

Conversational AI是新的付費媒體介面，[!DNL GenStudio for Performance Marketing]現在會端對端使用ChatGPT。 [建立具有品牌內副本變化的ChatGPT廣告體驗](/help/user-guide/create/create-chatgpt-ad.md)、執行品牌和管道檢查、透過核准路由工作，以及將核准的體驗發佈到[!DNL Content]以供重複使用和啟用。
...
[!DNL Create]現在支援Horizon畫布上的[快速範本](/help/user-guide/templates/express-templates.md)，適用於Meta、LinkedIn和Display。 您可以直接在建立工作流程中使用Adobe Express範本，包括入門範本和自訂品牌配置。

畫布支援範本功能，包括：

* 每個範本多種大小
* 在Express中設定的圖層鎖定與解鎖選項
* WYSIWYG編輯範本元素
* 更豐富的文字、媒體交換、標誌取代、視訊重新框架等控制項

在[使用GenStudio for Performance Marketing中的Adobe Express範本](/help/user-guide/templates/express-templates.md)中探索詳細資料。
...
* 直接在GenStudio for Performance Marketing中將核准的付費媒體和電子郵件體驗立即翻譯成40多種語言。 請參閱[翻譯及本地化體驗](/help/user-guide/create/translate-experiences.md)。
...
* 使用者現在可以套用[!DNL Brand]標籤，以便由[!DNL Brand]將[!DNL Product]和[!DNL Persona]記錄分組。
...
新的GenExpand功能可讓行銷人員動態調整其創意資產，以支援Meta、LinkedIn、顯示廣告和橫幅等付費媒體管道的不同外觀比例。 當影像不符合預期外觀比例時（例如將窄影像新增至寬版面），您可以使用GenExpand來修改影像以符合大小。
...
現在支援具有相同角色的多個文字欄位（例如，「內文」、「cta」、「影像文字」等） 適用於複雜的客戶範本。
...
**Firefly影像4**：針對速度與簡易性而最佳化，適合製作插圖、圖示、基本物件像片及單一主題肖像，滿足90%的日常創作需求。

**Firefly Image 4 Ultra**：優先處理真實感和精確度，擅長呈現人體肖像、中等大小的群組，以及高階創意工作的複雜場景。
...
[!DNL Insights]模組中的新篩選器會自動從廣告預覽中隱藏[不支援的影像和視訊資產](/help/user-guide/insights/published-experiences.md#ad-formats)，消除視覺雜訊和損毀的圖磚。 此增強功能可確保使用者只看到實際可用且可立即使用的媒體，進而建立更乾淨且更可靠的體驗。 此篩選器可與現有的相容性廣告篩選器搭配使用。
...
[使用RTF格式編輯產生之變體中的文字欄位](/help/user-guide/create/manage-variants.md#manually-edit-text)選項包括粗體、斜體、底線、文字對齊、清單、文字色彩、文字大小和連結。 這可讓您調整對象的文字和詞句，並套用格式以符合版面需求。

### 影像和連結的協助工具標籤

將協助工具標籤（Aria標籤）新增至變體中的影像和call-to-action連結，提供可協助使用者瞭解互動元素用途的存取名稱。 如需詳細指示，請參閱[管理變體](/help/user-guide/create/manage-variants.md)。
...
* 您現在可以在平台設定期間，透過複製平台設定期間的現有廣告集[建立新的廣告集](/help/user-guide/activation/activate-meta-ad.md#create-a-new-ad-set)。 Meta廣告集定義特定廣告的時機、頻道詳細資訊和對象。 Meta行銷活動可包含多個廣告集，但一個廣告集僅與一個行銷活動相關聯。
...
* 新的範本篩選選項！ 現在您可以在[!DNL Create]和&#x200B;_[!UICONTROL 內容]_ > _[!UICONTROL 範本]_&#x200B;中精簡您的&#x200B;_[!UICONTROL 選取範本]_&#x200B;清單。 請參閱[搜尋範本](/help/user-guide/templates/use-templates.md#search-templates)。 請確定範本已正確加上中繼資料標籤，如此才能透過這些篩選器探索範本。
...
* 已啟用[[!DNL Brands]](/help/user-guide/guidelines/brands.md)的[預設頻道](/help/user-guide/guidelines/brands.md#channel-guidelines)、[影像](/help/user-guide/guidelines/brands.md#image-guidelines)、[標誌](/help/user-guide/guidelines/brands.md#logos)和[色彩](/help/user-guide/guidelines/brands.md#colors)指引的功能。
...
您現在可以從任何[!DNL Insights]檢視將可檢視表格下載到CSV檔案中。 此功能可讓您從各種[!DNL Insights]檢視匯出和分析資料，方便資料分析和報告選項。
...
* 字元計數現在會顯示在顯示廣告的所有產生和手動欄位中。 檢視[Meta體驗](/help/user-guide/create/meta-experiences.md#character-counts)中的&#x200B;_字元計數_。<!-- GS-7732 -->
...
* 電子郵件call-to-action連結不再是必要欄位。<!-- GS-8103 -->
...
* Meta廣告的調整大小和重新產生作業問題已解決。<!-- GS-8900 -->
...
* 已本地化產生的電子郵件和Meta廣告中的`Zoom to fit to screen`字串。<!-- GS-5063 -->
...
* 調整多個Meta廣告變體大小現在能如預期般運作。 之前，「畫布」不會顯示重新產生的變體，但會保持空白。<!-- GS-7010 -->
...
GenStudio for Performance Marketing與其他Adobe CX Enterprise產品整合，包括Adobe Express和Adobe AEM Assets。
