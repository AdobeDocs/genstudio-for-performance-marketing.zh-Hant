...
![欄位角色對應](./field-role-mapping.png){width="60%"}
...
>[!NOTE]
>
> 若要執行此工作，您同時需要GenStudio for Performance Marketing的Figma外掛程式和[Photoshop外掛程式](photoshop-plugin.md)。

您可以使用Figma外掛程式，將Figma框架、多個框架或整個檔案轉換為Photoshop格式，並將其匯出以與[Photoshop外掛程式](photoshop-plugin.md)搭配使用。 目前，轉換期間僅支援主要屬性，例如可見度、字型大小以及基本圖層屬性。 目前尚不支援刪除線、上標、下標、百分比形式的不透明度、漸層和類似的進階屬性等功能。
...
1. 執行下列其中一項：
   ...
   * 按一下&#x200B;**[!UICONTROL 傳輸至Photoshop外掛程式]**，快取轉換後的檔案以立即用於Photoshop外掛程式。
     ![轉移到Photoshop外掛程式按鈕](./transfer-to-ps-button.png){width="60%"}
...
1. 按一下&#x200B;**[!UICONTROL 提交]**。 外掛程式會讀取圖形中選取的影格，並將其轉換為JSON檔案，這是檔案資料的中介格式。
   ![需要檔案金鑰對話方塊](./file-key-required.png){width="40%"}
1. 在Photoshop中，開啟GenStudio外掛程式，然後按一下「**[!UICONTROL 匯入]**」標籤。
1. 執行下列其中一項：

   * 按一下&#x200B;**[!UICONTROL 從外掛程式]**，從快取檔案清單中選擇以&#x200B;**[!UICONTROL 傳輸到Photoshop外掛程式]**&#x200B;轉換的檔案，或
   * 按一下&#x200B;**[!UICONTROL 上傳JSON]**&#x200B;以瀏覽並選取要上傳的JSON檔案。
     ![上傳JSON按鈕](./upload-json.png){width="40%"}
1. Photoshop中的GenStudio外掛程式會將JSON檔案中的資訊轉換為開啟的Photoshop檔案。
