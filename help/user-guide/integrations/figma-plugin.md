...
1. 選取&#x200B;**[!UICONTROL 產生影像]**。
1. 從下拉式選單中選取模型。 您也可以選擇已建立的任何自訂模型。
1. 選取設定圖示可調整產生設定。
1. 可選：選取外觀比例。
1. 可選：執行下列任一項作業來調整影像的樣式：
   * 選取&#x200B;**[!UICONTROL 上傳影像]**，從您的裝置或AEM上傳參考影像。
   * 選取&#x200B;**[!UICONTROL 瀏覽相簿]**，從Adobe的庫存影像中選擇一個。
   * 使用滑桿選擇強度值。 強度會調整Firefly嚴格遵循您所提供樣式的程度。
1. 選取&#x200B;**&lt;**&#x200B;按鈕。
1. 輸入提示。
1. 選取產生圖示。 影像會出現在外掛程式面板中。
1. 使用下列其中一種方法將影像移至畫布上：
   * 將任何影像拖放至畫布上。
   * 在「影象」畫布上選取影格，然後在外掛程式視窗中選取要插入影格的影像。
   * 選取上傳圖示以將影像上傳到畫布上。
   * 選取3個點，然後&#x200B;**[!UICONTROL 全部下載到Figma]**。
1. 可選：選取這3個點以執行進一步動作：
   * 選取&#x200B;**[!UICONTROL 產生更多]**&#x200B;以再次執行提示。
   * 選取&#x200B;**[!UICONTROL 複製提示]**&#x200B;以複製提示。
1. 可選：選取鉛筆圖示以在單一影像上使用「產生」填色和「產生類似動作」。

## 產生類似影像

產生一組類似的影像。

1. 選取&#x200B;**[!UICONTROL 產生類似]**&#x200B;卡片。
1. 執行下列任一項作業，選取影像作為參照：
   * 在圖形畫布上選取影像。
   * 選取&#x200B;**[!UICONTROL 上傳影像]**&#x200B;以從您的裝置上傳。
   * 選取&#x200B;**[!UICONTROL 瀏覽AEM資產]**，以從AEM上傳。
1. 選取產生圖示。 變數會出現在外掛程式面板中。
1. 使用下列其中一種方法將影像移至畫布上：
   * 將任何影像拖放至畫布上。
   * 在「影象」畫布上選取影格，然後在外掛程式視窗中選取要插入影格的影像。
   * 選取上傳圖示以將影像上傳到畫布上。
...
1. 選取&#x200B;**[!UICONTROL 移除背景]**&#x200B;卡片。
1. 執行下列任一項作業，選取影像作為參照：
   * 在圖形畫布上選取影像。
   * 選取&#x200B;**[!UICONTROL 上傳影像]**&#x200B;以從您的裝置上傳。
   * 選取&#x200B;**[!UICONTROL 瀏覽AEM資產]**，以從AEM上傳。
1. 選取&#x200B;**[!UICONTROL 移除]**。 如果從畫布中選取影像，則會取代圖形畫布上的影像。 如果影像是從裝置或AEM中選取，您可以將影像拖放到畫布上，或選取&#x200B;**[!UICONTROL 插入影像]**&#x200B;將影像放在畫布上。

## 產生式填色

為影像區域套用產生式填色。

1. 選取&#x200B;**[!UICONTROL 產生式填滿]**&#x200B;卡。
1. 執行下列任一項作業，選取影像作為參照：
   * 在圖形畫布上選取影像。
   * 選取&#x200B;**[!UICONTROL 上傳影像]**&#x200B;以從您的裝置上傳。
   * 選取&#x200B;**[!UICONTROL 瀏覽AEM資產]**，以從AEM上傳。
1. 選取筆刷工具並建立遮色片。
1. 可選：選取下拉式插入號並調整筆刷大小。
1. 選取「重設」按鈕以移除遮色片。
1. 或者，選取移除背景圖示以移除背景。
1. 輸入提示以引導產生選取的遮罩，並選取&#x200B;**[!UICONTROL 產生]**&#x200B;按鈕。
1. 使用下列其中一種方法將影像移至畫布上：
   * 將任何影像拖放至畫布上。
   * 在「影象」畫布上選取影格，然後在外掛程式視窗中選取要插入影格的影像。
   * 選取上傳圖示以將影像上傳到畫布上。
...
1. 選取設定圖示可調整產生設定。
1. 選用：選取外觀比例並選取&#x200B;**&lt;**&#x200B;按鈕。
1. 輸入提示以引導產生，並選取&#x200B;**[!UICONTROL 產生]**&#x200B;按鈕。
1. 使用下列其中一種方法將影像移至畫布上：
   * 將任何影像拖放至畫布上。
   * 在「影象」畫布上選取影格，然後在外掛程式視窗中選取要插入影格的影像。
   * 選取上傳圖示以將影像上傳到畫布上。
...
展開影像的尺寸，並以「創作」展開來新增創作的內容。 產生式展開可讓您將不符合的影像轉換為橫幅、Meta廣告、LinkedIn廣告或顯示廣告範本最適合的外觀比例。

1. 選取&#x200B;**[!UICONTROL 產生式展開]**&#x200B;卡片。
1. 在畫布上選取影像。
1. 將Gen Expand Temporary框架調整為您所需的新維度。
1. 可選：將影像移到框架內的任何位置。
1. 輸入提示以引導產生，並選取&#x200B;**[!UICONTROL 產生]**&#x200B;按鈕。
1. 在畫布上選取任何影像，以產生的結果取代原始影像。
