...
| 支援 | 不支援 |
|----------------------------------|-----------------------------------|
| 動態廣告<br>連結廣告<br>簡單影像廣告<br>簡單視訊廣告 | 購物廣告<br>Advantage+購物廣告<br>Advantage+目錄廣告<br>輪播廣告<br>Messenger廣告<br>潛在客戶廣告<br>集合廣告<br>通話廣告<br>擁有的頁面貼文廣告<br>合作關係廣告<br>彈性廣告 |

### 廣告刊登

廣告投放位置是指您的廣告在行銷活動中出現的特定位置或平台。 這些位置會決定您的對象與內容互動的方式和位置。 廣告刊登位置可擴大對象觸及面，有助最大化可見度、參與度和整體行銷活動的成效。

當您建立包含中繼廣告的行銷活動時，您可能已根據行銷活動[目標](channels.md#objectives)選擇執行廣告的位置。

以下是支援的廣告投放位置清單：

| 對象網路 | Facebook/Meta \* | Instagram | Messenger |
|--------------------|--------------------|-------------------------|---------------------|
...
\*在&#x200B;_中繼業務說明中心_&#x200B;中檢視[關於跨中繼技術的廣告刊登](https://www.facebook.com/business/help/407108559393196?id=369787570424415)。
...
| **[!UICONTROL CPM ]**<br>_每千成本_ | 每1,000次廣告印象的平均成本。<br>**計算**：總金額`spent`除以範圍，然後乘以1000 | 低值可能表示具有成本效益的可見度，尤其是與高點進率配對時。 |
| **[!UICONTROL CPA ]**<br>_每個動作的成本_ | 完成特定客戶動作（例如購買或訂閱）所花費的平均成本。<br>**計算**：總金額`spent`除以已完成的客戶動作數目 | 用於監控可產生寶貴客戶動作的廣告支出。 |
| **[!UICONTROL CPC ]**<br>_每次點按成本_ | 與廣告投放中每次點按相關聯的平均成本。<br>**計算**：總金額`spent`除以`clicks` | 平均成本降低可能表示廣告支出符合成本效益，尤其是與轉換率增加相比。 |
