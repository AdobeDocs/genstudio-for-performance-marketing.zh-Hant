...
如果影像樣式識別為`photograph`，AI會分析`camera settings`、`camera proximity`和`Photography genres`的其他特徵。 這些特徵只適用於像片，可讓您更深入地瞭解影像的構成和品質。 檢視Adobe的[學習攝影](https://www.adobe.com/creativecloud/photography/discover/types-of-photography.html)中的&#x200B;_28種攝影樣式_，並了解熱門攝影型別和基本術語。
...
| 攝影機角度 | 相機用來擷取主旨的角度，這會影響檢視者對影像的感知和詮釋。 如果影像樣式為`photograph`，則會識別此特徵。 | `Low angle`，`High angle`，`Eye level`，`Neutral angle`，`Overhead view`，`Bird's eye view`<p>`Overhead view`的範例：<p>![管理費用檢視](/help/assets/category/image-camera-angle.png "管理費用短褲組"){width="200" zoomable="yes"} |
| 相機設定 | 相機控制項的特定調整和組態會影響影像的最終外觀和品質。 如果影像樣式為`photograph`，則會識別此特徵。 | `Fast shutter speed`、`Long exposure`、`Bokeh blur`、`Motion blur`、`Tilt-shift blur`、`Flash`、`Wide-angle`、`Black and white`、`Double-exposure`、`Macro`、`Normal mode`<p>`Fast shutter speed`設定的範例：<p>![快門速度](/help/assets/category/image-camera-setting.png "在波浪上衝浪"){width="200" zoomable="yes"} |
...
| 主題距離 | 相機與影像主體之間的距離。 | `close up`、`mid shot`、`long shot`<p>`Long shot`的範例：<p>![長距山頂](/help/assets/category/image-subject-distance.png "遠距山頂上的人"){width="200" zoomable="yes"} |
