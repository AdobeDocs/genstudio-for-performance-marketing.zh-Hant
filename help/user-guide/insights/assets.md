...
_[!UICONTROL Assets]_&#x200B;表格是使用[!UICONTROL 資產ID]來組織。 您可以使用檢視清單（表格）圖示和相簿檢視（格點）圖示，在檢視之間切換。 按一下表格右上方的設定(cog)圖示，以切換可檢視欄。 表格左上方的篩選器（漏斗）圖示會開啟&#x200B;**[!UICONTROL 篩選器]**&#x200B;功能表，您可在此從多個清單中選取。 按一下&#x200B;**重設**&#x200B;以清除所有篩選器選取專案。
...
| **[!UICONTROL CTR ]**<br>_點進率_ | 導致體驗內資產點按的曝光百分比(%)。<br>**計算**： `clicks`除以`impressions` | 高點進率表示內容與對象高度相關且吸引人。 這表示傳訊和設計有效地擷取了對象的興趣，並促使他們採取行動。 此外，高CTR可能表示資產目標鎖定準確，並與目標對象產生共鳴，進而提升整體行銷活動績效。 |
...
| **[!UICONTROL 支出]** | 預算中花費的金額，與指定期間內的個別資產有關。 | 在短時間內發生高支出量可能表示快速使用，這可能導致資源提早耗盡。 根據關鍵績效指標追蹤所花費的金額，以協助監控整體投資報酬率。 |
