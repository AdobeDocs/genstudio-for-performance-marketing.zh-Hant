...
[!DNL GenStudio for Performance Marketing]的Experience Selector指令碼是從Adobe在`experience.adobe.com`上代管的URL載入，而非從一般實作中的Salesforce靜態資源載入。
...
專案使用Salesforce DX配置；預設套件目錄為`force-app`。
...
存放庫也可能包含靜態資源(`reactApp`， `sfgsmfe_react`)。 `sfgsmfe.js`中的目前[!DNL GenStudio for Performance Marketing]載入器使用`standalone.js`的Adobe CDN URL；除非您變更實作，否則該載入路徑不需要這些靜態資源。
...
1. 依照Lightning的目前Salesforce檔案，新增或調整`https://experience.adobe.com`的&#x200B;**[!UICONTROL 信任URL]** （以及您Salesforce版本的任何相關設定）。
   ![Salesforce CSP信任的網站](./sf-trusted-urls.png){width="80%" zoomable="yes"}
...
1. 部署完成且沒有錯誤。
1. 使用者可以開啟包含`sfgsmfe`的Lightning頁面，並檢視Experience Selector UI。
1. 元件未顯示載入錯誤；[網路]索引標籤傳回`standalone.js`的HTTP 200。
1. **[!UICONTROL 選取GenStudio Experience]**&#x200B;會開啟選取器並執行選取範圍回呼。
1. 當您使用該流程時，**[!UICONTROL 建立電子郵件範本]**&#x200B;成功，範本出現在&#x200B;**[!UICONTROL 設定]**&#x200B;中已設定的資料夾下。
